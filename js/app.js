(function(){
  ...
  const T = {
    he: {
      ...
      "itin.note":"כל הזמנים הם דוגמה בלבד — כל טיסה נבנית לפי משך הזמן, היעד וההעדפות שתבחרו.",
      "trip.eyebrow":"בואו נטוס לשם","trip.h2":"שלושה טיולים מלאים","trip.p":"דוגמאות אמיתיות שמשלבות טיסה, נחיתה, ועצירה בקרקע בזמן שאתם בוחרים — בדיוק כמו שהמחשבון למעלה מאפשר.",
      "trip.flightTime":"זמן טיסה כולל","trip.groundTime":"עצירה בקרקע",
      "trip.haifa.tag":"מטוס 172 · טיול זוגי","trip.haifa.h":"חיפה","trip.haifa.p":"ממריאים מהרצליה לחיפה לאורך קו החוף וההרים הירוקים של הכרמל — כ-25 דקות טיסה בכל כיוון. נוחתים בשדה התעופה ויורדים לטיול זוגי: גני הבאהאים, שוק תלפיות, או חוף הכרמל — לבחירתכם, שעתיים-שלוש. עולים חזרה למטוס וטסים חזרה להרצליה.","trip.haifa.ground":"2–3 שעות","trip.haifa.price":"מחיר משוער: ₪1,770–1,830",
      "trip.roshpina.tag":"מטוס 172 · טיול גליל","trip.roshpina.h":"ראש פינה והגליל","trip.roshpina.p":"טסים צפונה מעל עמק הירדן והכנרת אל ראש פינה — כ-45 דקות טיסה בכל כיוון. נוחתים בשדה התעופה ההיסטורי ויורדים לסיור: המושבה העתיקה עם הסמטאות והגלריות, יקב בוטיק ברמת הגולן, או שייט קצר בכנרת — כ-3-4 שעות לבחירתכם. חוזרים לשדה וטסים בחזרה להרצליה.","trip.roshpina.ground":"3–4 שעות","trip.roshpina.price":"מחיר משוער: ₪2,660–2,720",
      "trip.masada.tag":"מטוס 172 · מדבר וים המלח","trip.masada.h":"מצדה וים המלח","trip.masada.p":"טסים דרומה מעל מדבר יהודה אל מנחת מצדה — כ-35 דקות טיסה בכל כיוון, עם נוף אווירי לים המלח שרק מעטים רואים. נוחתים ועולים ברכבל אל ההר המבודד והחפירות הארכיאולוגיות, ולסיום צפים בים המלח — כ-3-4 שעות. חוזרים למטוס וטסים בחזרה להרצליה.","trip.masada.ground":"3–4 שעות","trip.masada.price":"מחיר משוער: ₪2,240–2,300",
      ...
      "faq.q3":"כמה נוסעים יכולים לעלות לטיסה?","faq.a3":"בוחרים ישירות במחשבון בין ססנה 152 (טייס + נוסע אחד) לססנה 172 (טייס + עד 3 נוסעים) — מספר הנוסעים המדויק מאושר בטופס יצירת הקשר.",
      ...
      "faq.q6":"אפשר לנחות ולהישאר ביעד לזמן מה?","faq.a6":"כן — לאחר בחירת יעד, ניתן להוסיף עצירה על הקרקע (עד פי 2 מזמן הטיסה), בעלות של 60 ₪ לכל שעת עצירה. משך העצירה לבחירתכם, והמחיר מתעדכן במחשבון בזמן אמת.",
    ...
    en: {
      ...
      "faq.q3":"How many passengers can fly?","faq.a3":"Choose directly in the calculator between a Cessna 152 (pilot + 1 passenger) or a Cessna 172 (pilot + up to 3 passengers) — the exact passenger count is confirmed in the contact form.",
      ...
      "faq.q6":"Can we land and stay at the destination for a while?","faq.a6":"Yes — after choosing a destination, you can add a ground stopover (up to 2x the flight time), at ₪60 per hour. The stopover length is your choice, and the price updates live in the calculator.",
  ...
  const destinations = [
    ...
    { id:'haifa', min:1, he:'חיפה', en:'Haifa', field:'שדה תעופה חיפה',
    ...
    { id:'megiddo', min:1.25, he:'מגידו · עמק יזרעאל', en:'Megiddo · Jezreel Valley', field:'מנחת מגידו',
    ...
    { id:'masada', min:1.25, he:'מצדה · ים המלח', en:'Masada · Dead Sea', field:'מנחת מצדה',
    ...
    { id:'beersheva', min:1.25, he:'באר שבע · הנגב', en:'Beer Sheva · Negev', field:'מנחת באר-שבע',
    ...
    { id:'roshpina', min:1.5, he:'ראש פינה · הגליל', en:'Rosh Pina · Galilee', field:'שדה תעופה ראש פינה',
  ...
  document.getElementById('leadForm').addEventListener('submit', async function(e){
    ...
    // Also store a structured record in Supabase (used by the admin bookings/calendar page).
    // This is independent of the Formspree email above -- if one fails the other can still succeed.
    if(window.amiraDB){
      window.amiraDB.from('bookings').insert({
        customer_name: f_name.value.trim(),
        phone: f_phone.value.trim(),
        email: f_email.value.trim(),
        flight_date: document.getElementById('f_date').value || null,
        flight_type: typeNames[currentLang][selectedType],
        aircraft: currentAircraft(),
        passengers: parseInt(document.getElementById('f_pax').value, 10) || null,
        duration_hours: dur,
        destination: destObj ? (currentLang==='he'?destObj.he:destObj.en) : null,
        notes: document.getElementById('f_notes').value.trim(),
        estimated_price: total
      }).then(({error})=>{ if(error) console.error('booking insert error:', error); });
    }
  ...
  // ---------------- shared timeout helper (used by renderReviews) ----------------
